        ...
        if (kindsToBeAssigned & TR_FPR_Mask) {
            ...
            for (auto j = 0U; j < getDependencyConditions()->getNumPostConditions(); ++j) {
                ...
                if (vr && (vr->getKind() == TR_FPR || vr->getKind() == TR_VRF)) {
                    machine->setVirtualAssociatedWithReal(dep->getRealRegister(), vr);
                }
            }

            machine->setXMMWeightsFromAssociations();
        }
    }
}
...
void OMR::X86::Instruction::adjustVFPState(TR_VFPState *state, TR::CodeGenerator *cg)
{
    ...
    if (state->_register == TR::RealRegister::esp) {
        if (getOpCode().isPushOp())
            ...
            state->_displacement -= static_cast<int32_t>(TR::Compiler->om.sizeofReferenceAddress());
        else if (getOpCodeValue() == OP::RET || getOpCodeValue() == OP::RETImm2 || getOpCodeValue() == OP::retn)
...
bool OMR::X86::Instruction::isLabel() { return getOpCodeValue() == OP::label; }
