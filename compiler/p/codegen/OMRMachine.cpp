...
TR::RealRegister *OMR::Power::Machine::freeBestRegister(TR::Instruction *currentInstruction, TR::Register *virtReg,
    TR::RealRegister *forced, bool excludeGPR0)
        ...
        case TR_GPR:
            if (false /*(cg()->isOutOfLineColdPath() || cg()->isOutOfLineHotPath()) && virtReg->getBackingStorage()*/) {
                ...
                if (candidates[0]->getBackingStorage()) {
                    ...
                    if (location->getSymbolReference()->getSymbol()->getSize()
            ...
            if (false /*(cg()->isOutOfLineColdPath() || cg()->isOutOfLineHotPath()) && virtReg->getBackingStorage()*/) {
                ...
                if (candidates[0]->getBackingStorage()) {
                    ...
                    if (candidates[0]->isSinglePrecision()
                        && location->getSymbolReference()->getSymbol()->getSize() > 4)
                        location->setFirstHalfIsOccupied();
                    else
                        location->setIsOccupied();
                } else {
...
TR::RealRegister *OMR::Power::Machine::assignOneRegister(TR::Instruction *currentInstruction, TR::Register *virtReg,
    bool excludeGPR0)
{
...
void OMR::Power::Machine::coerceRegisterAssignment(TR::Instruction *currentInstruction, TR::Register *virtualRegister,
    ...
    TR::RealRegister *sourceReg, TR::CodeGenerator *cg)
{
    ...
    switch (rk) {
        case TR_GPR:
            instr = generateTrg1Src1Instruction(cg, TR::InstOpCode::mr, currentNode, targetReg, sourceReg,
                precedingInstruction);
            break;
        case TR_FPR:
            if (useVSXLogical)
                instr = generateTrg1Src2Instruction(cg, TR::InstOpCode::xxlor, currentNode, targetReg, sourceReg,
                    sourceReg, precedingInstruction);
            else
                instr = generateTrg1Src1Instruction(cg, TR::InstOpCode::fmr, currentNode, targetReg, sourceReg,
                    precedingInstruction);
            break;
        case TR_CCR:
            instr = generateTrg1Src1Instruction(cg, TR::InstOpCode::mcrf, currentNode, targetReg, sourceReg,
                precedingInstruction);
            break;
        case TR_VSX_SCALAR:
        case TR_VSX_VECTOR:
    ...
    TR::RealRegister *sourceReg, TR::RealRegister *middleReg, TR::CodeGenerator *cg)
{
    ...
    if (middleReg != NULL) {
        ...
        cg->traceRAInstruction(generateTrg1Src2Instruction(cg, opCode, currentNode, targetReg, targetReg, sourceReg,
            precedingInstruction));
        cg->traceRAInstruction(generateTrg1Src2Instruction(cg, opCode, currentNode, sourceReg, targetReg, sourceReg,
            precedingInstruction));
        cg->traceRAInstruction(generateTrg1Src2Instruction(cg, opCode, currentNode, targetReg, targetReg, sourceReg,
            precedingInstruction));
    }
