...
void OMR::X86::Linkage::mapCompactedStack(TR::ResolvedMethodSymbol *method)
{
        ...
        if (localCursor->getGCMapIndex() >= 0) {
            ...
            if (igNode) {
                ...
                if (localCursor->isInternalPointer() || localCursor->isPinningArrayPointer()
                    ...
                    performSharing
                        = performTransformation(comp, "O^O COMPACT LOCALS: Sharing slot for local %p\n", localCursor);
    ...
    if (debug("reportCL")) {
        ...
        diagnostic("\n**** Mapped locals size: %d (orig map size=%d, shared size=%d)  %s\n",
            ...
            origSize - (linkage.getOffsetToFirstLocal() - stackIndex), comp->signature());
        ...
        diagnostic("\n**** Accumulated totals: mapped size=%d, shared size=%d, original size=%d after %s\n",
            accumMappedSize, accumOrigSize - accumMappedSize, accumOrigSize, comp->signature());
#endif
    }
}
...
void OMR::X86::Linkage::associatePreservedRegisters(TR::RegisterDependencyConditions *deps,
...
TR::Register *OMR::X86::Linkage::findReturnRegisterFromDependencies(TR::Node *callNode,
    TR::RegisterDependencyConditions *dependencies, TR::MethodSymbol *methodSymbol, bool isDirect)
{
...
uint8_t *OMR::X86::Linkage::loadArguments(TR::Node *callNode, uint8_t *cursor, bool calculateSizeOnly,
    int32_t *sizeOfFlushArea, bool isReturnAddressOnStack)
{
    return self()->flushArguments(callNode, cursor, calculateSizeOnly, sizeOfFlushArea, isReturnAddressOnStack, true);
}

uint8_t *OMR::X86::Linkage::storeArguments(TR::Node *callNode, uint8_t *cursor, bool calculateSizeOnly,
