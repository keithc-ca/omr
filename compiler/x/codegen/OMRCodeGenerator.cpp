...
void OMR::X86::CodeGenerator::initializeX86(TR::Compilation *comp)
{
    ...
    self()->setLastGlobalGPR(machine()->getLastGlobalGPRRegisterNumber());
    self()->setLast8BitGlobalGPR(machine()->getLast8BitGlobalGPRRegisterNumber());
    self()->setLastGlobalFPR(machine()->getLastGlobalFPRRegisterNumber());
    /*
   ...
   if (comp->target().is64Bit())
    ...
    _unlatchedRegisterList = (TR::RealRegister **)trMemory()->allocateHeapMemory(
        sizeof(TR::RealRegister *) * (TR::RealRegister::NumRegisters + 1));
    _unlatchedRegisterList[0] = 0; // mark that list is empty

    self()->setGlobalRegisterTable(machine()->getGlobalRegisterTable(*_linkageProperties));
    ...
    //
    _frameRegister = machine()->getRealRegister(TR::RealRegister::vfp);
    ...
    if (vmThreadIndex != TR::RealRegister::NoReg) {
        TR::RealRegister *vmThreadReal = machine()->getRealRegister(vmThreadIndex);
        ...
        while (iterator != self()->getLiveDiscardableRegisters().end()) {
            ...
            if (registerCursor->getRematerializationInfo()->isRematerializableFromMemory()) {
                ...
                if ((rmSymRef->getSymbol() == symbol)
                    ...
                    if (!clob) {
                        clob = new (self()->trHeapMemory()) TR::ClobberingInstruction(instr, trMemory());
                    ...
                    if (debug("dumpRemat")) {
                        diagnostic("---> Clobbering %s discardable register %s at instruction %p in %s\n",
                            ...
                            getDebug()->getName(registerCursor), instr, comp()->signature());
                    }
                }
...
void OMR::X86::CodeGenerator::reactivateDependentDiscardableRegisters(TR::Register *baseReg)
{
    ...
    if (debug("dumpRemat"))
        diagnostic("---> Re-activating rematerializable registers dependent on %s: ", getDebug()->getName(baseReg));
...
void OMR::X86::CodeGenerator::deactivateDependentDiscardableRegisters(TR::Register *baseReg)
{
    ...
    if (debug("dumpRemat"))
        diagnostic("---> Deactivating rematerialisable registers dependent on %s: ", getDebug()->getName(baseReg));
...
{
    TR::CPU *cpu = &comp()->target().cpu;

    return comp()->getOption(TR_EnableOpMaskRegisters) && cpu->supportsFeature(OMR_FEATURE_X86_AVX512F)
...
TR::VectorLength OMR::X86::CodeGenerator::getMaxPreferredVectorLength()
{
    TR::CPU *cpu = &comp()->target().cpu;
...
bool OMR::X86::CodeGenerator::getSupportsOpCodeForAutoSIMD(TR::CPU *cpu, TR::ILOpCode opcode, bool supportsOpMaskRegs)
{
    ...
    switch (opcode.getVectorOperation()) {
        ...
        case TR::vgetelem:
#if 0
         if (comp()->target().is64Bit() && (et == TR::Int32 || et == TR::Int64 || et == TR::Float || et == TR::Double))
            return true;
         else
#endif // closes the if 0
            return false;
    ...
    TR::Instruction *instructionCursor, TR::Instruction *appendInstruction)
{
    ...
    if (self()->getUseNonLinearRegisterAssigner()) {
        if (!getSpilledRegisterList()) {
            setSpilledRegisterList(new (self()->trHeapMemory())
                    TR::list<TR::Register *>(getTypedAllocator<TR::Register *>(comp->allocator())));
        }
    }

    if (getDebug())
        getDebug()->startTracingRegisterAssignment(comp->log(), kindsToAssign);
...
void OMR::X86::CodeGenerator::doRegisterAssignment(TR_RegisterKinds kindsToAssign)
{
    ...
    if (kindsToAssign) {
        ...
        self()->doBackwardsRegisterAssignment(kindsToAssign, getAppendInstruction());
    }
...
void OMR::X86::CodeGenerator::doBinaryEncoding()
{
    ...
    logprints(trace, log, "<proepilogue>\n");
    ...
    for (TR::Instruction *gcMapCursor = prologueCursor; gcMapCursor != _vfpResetInstruction;
        ...
        if (gcMapCursor->needsGCMap())
            gcMapCursor->setGCMap(self()->getStackAtlas()->getParameterMap()->clone(trMemory()));
    }
    ...
    while (estimateCursor) {
        ...
        if (comp->getOption(TR_TraceCG))
            getDebug()->dumpInstructionWithVFPState(comp->log(), estimateCursor, &prevState);
...
#define OVER_ESTIMATION 4
    ...
    while (cursorInstruction) {
        ...
        if (cursorInstruction->isLastWarmInstruction()) {
            ...
            logprintf(trace, log, "%s warmCodeEnd = %p, lastWarmInstruction = %p coldCodeStart = %p\n",
    ...
    if (comp->getOption(TR_EnableGCRPatching) && comp->getRecompilationInfo()
        && comp->getRecompilationInfo()->getJittedBodyInfo()->getUsesGCR()) {
        void *addrToPatch = comp->getSymRefTab()
        ...
        if (!addrToPatch) {
            ...
            comp->failCompilation<TR::GCRPatchFailure>(
...
TR::Register *OMR::X86::CodeGenerator::gprClobberEvaluate(TR::Node *node, OP::Mnemonic movRegRegOpCode)
{
    ...
    if (sourceRegister->needsLazyClobbering())
        logprintf(trace, log, "LAZY CLOBBERING: node %s register %s refcount=%d canClobber=%s\n",
            getDebug()->getName(node), getDebug()->getName(sourceRegister), node->getReferenceCount(),
    ...
    if (canClobber) {
        ...
        if (sourceRegister->containsCollectedReference()) {
            logprintf(trace, log, "Setting containsCollectedReference on register %s\n",
                getDebug()->getName(targetRegister));
        ...
        if (sourceRegister->containsInternalPointer()) {
            ...
            logprintf(trace, log,
                ...
                getDebug()->getName(targetRegister), pinningArrayPointer);
    ...
    TR::SymbolReference *helper)
{
    ...
    if (self()->directCallRequiresTrampoline(helperAddress, (intptr_t)branchInstructionAddress)) {
        ...
        TR_ASSERT_FATAL(comp()->target().cpu.isTargetWithinRIPRange(helperAddress, (intptr_t)nextInstructionAddress),
...
TR::RealRegister::RegNum OMR::X86::CodeGenerator::pickNOPRegister(TR::Instruction *successor)
{
    ...
    TR::RealRegister *ebx = machine()->getRealRegister(TR::RealRegister::ebx);
    TR::RealRegister *esi = machine()->getRealRegister(TR::RealRegister::esi);
    TR::RealRegister *edi = machine()->getRealRegister(TR::RealRegister::edi);
...
{
    OMR::Logger *log = comp()->log();
    bool trace = self()->traceSimulateTreeEvaluation();
    ...
    if (node->getFutureUseCount() > 1) {
        ...
        if (parent->getOpCode().isBndCheck() && node->getOpCode().isArrayLength() && node->getFutureUseCount() == 2) {
            if (result)
                logprints(trace, log, " bndchk/arraylength");
    ...
    if (result)
        logprintf(trace, log, "%s foldable into %s", getDebug()->getName(node), getDebug()->getName(parent));
    ...
    TR_RegisterPressureSummary *summary)
{
    ...
    if ((opCode.isMul() || opCode.isDiv() || opCode.isRem()) && !(opCode.isFloat() || opCode.isDouble())) {
        ...
        if (usesMul) {
            ...
            logprintf(trace, log, " mul:g=%d", summary->_gprPressure);
        }
    ...
    TR_PaddingProperties properties, bool recursive)
{
    ...
    if (length <= _paddingTable->_biggestEncoding) {
        ...
        if (_paddingTable->_flags.testAny(TR_X86PaddingTable::registerMatters)) {
            ...
            TR::RealRegister *reg = machine()->getRealRegister(regIndex);
        ...
        if (neighborhood) {
            ...
            if (neighborhood->getKind() == TR::Instruction::IsVirtualGuardNOP && guardNode
                ...
                TR::DebugCounter::incStaticDebugCounter(comp,
                    TR::DebugCounter::debugCounterName(comp, "nopCount/%d/%s/%s", blockFrequency,
                        neighborhood->description(), guardKind));
                if (length > 0) {
                    TR::DebugCounter::incStaticDebugCounter(comp,
                        TR::DebugCounter::debugCounterName(comp, "nopInst/%d/%s/%s", blockFrequency,
                            neighborhood->description(), guardKind));
                    ...
                    for (TR::Instruction *ninst = neighborhood->getNext(); ninst; ninst = ninst->getNext()) {
                        if (ninst->isVirtualGuardNOPInstruction()) {
                            if (guardNode->isHCRGuard() && ninst->getNode() && ninst->getNode()->isHCRGuard()
                                ...
                                continue;
                            TR::DebugCounter::incStaticDebugCounter(comp,
                                TR::DebugCounter::debugCounterName(comp, "vgnopNoPatchReason/%d/vgnop",
                                    blockFrequency));
                            break;
                        }
                        if (comp->isPICSite(ninst)) {
                            TR::DebugCounter::incStaticDebugCounter(comp,
                                TR::DebugCounter::debugCounterName(comp, "vgnopNoPatchReason/%d/staticPIC",
                                    blockFrequency));
                            break;
                        }
                        if (ninst->isPatchBarrier(self())) {
                            if (ninst->getOpCodeValue() != OP::label)
                                ...
                                TR::DebugCounter::incStaticDebugCounter(comp,
                                    TR::DebugCounter::debugCounterName(comp, "vgnopNoPatchReason/%d/controlFlowMerge",
                                        blockFrequency));
                            break;
                        }
                        if (ninst->getEstimatedBinaryLength() > 0) {
                            TR::DebugCounter::incStaticDebugCounter(comp,
                                TR::DebugCounter::debugCounterName(comp, "vgnopNoPatchReason/%d/%s_%s", blockFrequency,
                                    (guardNode->isHCRGuard() ? "hcr" : "nohcr"), ninst->description()));
                        }
                    }
                }
            } else {
                TR::DebugCounter::incStaticDebugCounter(comp,
                    TR::DebugCounter::debugCounterName(comp, "nopCount/%d/%s", blockFrequency,
                        neighborhood->description()));
                if (length > 0)
                    TR::DebugCounter::incStaticDebugCounter(comp,
                        TR::DebugCounter::debugCounterName(comp, "nopInst/%d/%s", blockFrequency,
                            neighborhood->description()));
            }
        } else {
...
void OMR::X86::CodeGenerator::dumpPreGPRegisterAssignment(OMR::Logger *log, TR::Instruction *instructionCursor)
{
    if (instructionCursor->totalReferencedGPRegisters(self()) > 0) {
        ...
        getDebug()->print(log, instructionCursor);
        getDebug()->printReferencedRegisterInfo(log, instructionCursor);

        if (debug("dumpGPRegStatus")) {
            machine()->printGPRegisterStatus(log, fe(), machine()->registerFile());
        }
    }
}
    ...
    TR::Instruction *origNextInstruction)
{
    if (instructionCursor->totalReferencedGPRegisters(self()) > 0) {
        ...
        while (nextInstruction && (nextInstruction != origNextInstruction)) {
            getDebug()->print(log, nextInstruction);
...
bool OMR::X86::CodeGenerator::directCallRequiresTrampoline(intptr_t targetAddress, intptr_t sourceAddress)
{
    ...
    //
    return !comp()->target().cpu.isTargetWithinRIPRange(targetAddress, sourceAddress + 5)
...
void OMR::X86::CodeGenerator::moveOutOfLineInstructionsToWarmCode()
{
    ...
    TR::Compilation *comp = self()->comp();
    logprintf(comp->getOption(TR_TraceCG), comp->log(), "Moving OutOfLine instructions to after %p\n",
    ...
    while (oiIterator != self()->getOutlinedInstructionsList().end()) {
        ...
        TR_ASSERT_FATAL(self()->getLastWarmInstruction() != getAppendInstruction(),
        ...
        if (self()->getLastWarmInstruction() != firstOLInstruction->getPrev()) {
            ...
            if (lastOLInstruction == getAppendInstruction())
                setAppendInstruction(firstOLInstruction->getPrev());
